...
def train_phase(model, optimizer, all_inputs, phase, num_cot_tokens,
                ...
                heads=None, num_targets=1, global_step=0, training_history=None,
    ...
    Args:
        ...
        num_targets: total number of targets (1 + len(flipping_bits))
    ...
    while eval_loss > target_loss and iter_num < max_iterations:
        ...
        optimizer.zero_grad()
        heads_list = [heads[current_num_cot][j] for j in range(num_targets)]
        outputs, loss = generate_with_multi_heads(model, modified_batch, current_num_cot, heads_list, targets_all=targets_all)
        ...
        if (iter_num + 1) % eval_interval == 0:
            eval_loss, accuracy, r_current, per_target_losses = evaluate(
                model,
                all_inputs,
                phase,
                num_cot_tokens,
            ...
            for j, tl in per_target_losses.items():
                log_dict[f"eval_loss_target_{j}"] = tl
            wandb.log(log_dict)
            ...
            if training_history is not None:
                ...
                for j, tl in per_target_losses.items():
                    entry[f"eval_loss_target_{j}"] = tl
                training_history.append(entry)
            
            target_strs = " | ".join(f"t{j}={v:.4f}" for j, v in per_target_losses.items())
            print(f"  Phase {phase}, Iter {iter_num + 1}: Train={avg_train_loss:.4f}, Eval={eval_loss:.4f}, MSE={accuracy:.4f}, r={r_current} [{target_strs}]")
            model.train()
    ...
    eval_loss, accuracy, _, per_target_losses = evaluate(
        model,
        all_inputs,
        phase,
        num_cot_tokens,
    ...
    target_strs = " | ".join(f"t{j}={v:.4f}" for j, v in per_target_losses.items())
    print(f"Phase {phase} Complete: Loss={eval_loss:.4f}, MSE={accuracy:.4f} (after {iter_num} iters) [{target_strs}]\n")
...
def generate_with_multi_heads(model, idx, num_cot_tokens, heads_list, targets_all=None):
    """
    Generate num_cot_tokens thinking CoT tokens, then len(heads_list) sequential
    prediction CoT tokens. Prediction token j uses heads_list[j] to predict
    targets_all[:, j].

    Args:
        model: GPT model
        idx: (B, input_length) input token indices
        num_cot_tokens: number of thinking CoT tokens (= phase in curriculum)
        heads_list: list of num_targets linear heads, one per target
        targets_all: (B, num_targets) tensor of all targets, or None

    Returns:
        outputs: list of num_targets output tensors, each (B,)
        loss: average MSE across all targets (or None)
    """
    B, input_length = idx.size()
    num_pred = len(heads_list)
    truncate_backprop = getattr(model.config, 'truncate_backprop', False)
    total_steps = num_cot_tokens + num_pred
    backprop_steps = getattr(model.config, 'backprop_steps', total_steps)
    detach_until = total_steps - backprop_steps if truncate_backprop else 0
    ...
    for j in range(num_pred):
        step = num_cot_tokens + j
        if truncate_backprop and step < detach_until:
            cot_embeddings = cot_embeddings.detach()
        ...
        output = heads_list[j](pred_hidden).squeeze(-1)
        outputs.append(output)

        if targets_all is not None:
            losses.append(nn.functional.mse_loss(output, targets_all[:, j]))
...
@torch.no_grad()
def evaluate(model, all_inputs, phase, num_cot_tokens, detect_threshold=0.2, show_examples=0, eval_batch_size=1024, subset_indices_arr=None, coefficients=None, flipping_bits=None, heads=None, num_targets=1):
    """Evaluate model using generate_with_multi_heads on all targets simultaneously.

    Returns:
        loss: average MSE across all targets at the current phase
        accuracy: same as loss (MSE indicator)
        r: detected curriculum level
        per_target_losses: dict {target_idx: mse} at the current phase
    """
    model.eval()
    ...
    for cot_tokens in range(1, phase + 1):
        ...
        heads_list = [heads[cot_tokens][j] for j in range(num_targets)]
        outputs_list, avg_loss = generate_with_multi_heads(
            model, modified_inputs, cot_tokens, heads_list, targets_all=targets_all
        )
        losses_by_cot[cot_tokens] = avg_loss.item()
        if cot_tokens == phase:
            for j in range(num_targets):
                per_target_losses_phase[j] = nn.functional.mse_loss(
                    outputs_list[j], targets_all[:, j]
                ).item()
            outputs_phase = outputs_list[0]
            targets_phase = targets_all[:, 0]
    ...
    if show_examples > 0:
        print(f"\n  Sample predictions (phase {phase}, original target, {phase} thinking + {num_targets} pred CoT tokens):")
        ...
        target_strs = " | ".join(f"t{j}={v:.4f}" for j, v in per_target_losses_phase.items())
        print(f"  Per-target losses: [{target_strs}]")

    return loss, accuracy, r, per_target_losses_phase
...
def main():
    ...
    parser.add_argument('--flipping_bits', type=str, default='0,1', help='Comma-separated bit indices to flip for extra targets')
    ...
    print(f'flipping bits: {flipping_bits} -> {num_targets} targets ({num_targets} prediction CoT tokens per phase)')
    ...
    # input (n_bits) + thinking CoT (n_bits max) + prediction CoT (num_targets) + margin
    block_size = args.n_bits + args.n_bits + num_targets + 2
    config = GPTConfig(
    ...
    for phase in range(1, args.n_bits + 1):
        print(f"{'='*60}")
        if args.random_subset:
            print(f"PHASE {phase}: Learning Fourier parity with {phase} thinking + {num_targets} prediction CoT tokens")
        else:
            print(f"PHASE {phase}: Learning parity of first {phase} bits with {phase} thinking + {num_targets} prediction CoT tokens")
        print(f"{'='*60}")
    ...
    for phase in range(1, args.n_bits + 1):
        loss, accuracy, _, per_target_losses = evaluate(
            model,
            all_inputs,
            phase,
            num_cot_tokens=phase,
        ...
        target_strs = " | ".join(f"t{j}={v:.4f}" for j, v in per_target_losses.items())
        if args.random_subset:
            print(f"Phase {phase} (Fourier, {phase}+{num_targets} CoT): Loss={loss:.4f}, MSE={accuracy:.4f} [{target_strs}]\n")
        else:
            print(f"Phase {phase} (parity of {phase} bits, {phase}+{num_targets} CoT): Loss={loss:.4f}, MSE={accuracy:.4f} [{target_strs}]\n")
