...
def compute_parity(inputs, k, subset_indices_arr=None, coefficients=None):
    ...
    Returns:
        result: tensor of parity values (B,)
...
def train_phase(model, optimizer, all_inputs, phase, num_cot_tokens,
    ...
    while eval_loss > target_loss and iter_num < max_iterations:
        ...
        if multilevel_training:
            ...
            target, modified_batch = compute_parity(batch, i, subset_indices_arr=subset_indices_arr, coefficients=coefficients)
            # Use i CoT tokens
            current_num_cot = i
            k_for_parity = i
        else:
            # Compute target parity and get modified batch for this phase
            target, modified_batch = compute_parity(batch, phase, subset_indices_arr=subset_indices_arr, coefficients=coefficients)
            current_num_cot = num_cot_tokens
            k_for_parity = phase
        
        # Debug: print num_cot and k used (only once per eval_interval)
        if iter_num % eval_interval == 0:
            print(f"  [TRAIN DEBUG] iter={iter_num}, k_for_parity={k_for_parity}, num_cot_tokens={current_num_cot}, phase={phase}, num_cot_tokens_arg={num_cot_tokens}")
        ...
        # Evaluate on random batch periodically (always evaluate on the current phase target)
...
@torch.no_grad()
def evaluate(model, all_inputs, n_bits, phase, num_cot_tokens, detect_threshold=0.2, show_examples=0, eval_batch_size=1024, subset_indices_arr=None, coefficients=None):
    """Evaluate model on a random batch of inputs.
    
    Args:
        model: GPT model
        all_inputs: all possible inputs (kept for easy revert)
    ...
    # Use sample_random_batch for evaluation (same as training)
    # To revert: comment the line below and uncomment the two lines after it
    eval_inputs = sample_random_batch(n_bits, eval_batch_size)
    # num_samples = all_inputs.size(0)
    # eval_inputs = all_inputs[torch.randint(0, num_samples, (min(eval_batch_size, num_samples),), device=all_inputs.device)]
    
    #print(f"  [EVAL DEBUG] phase={phase}, num_cot_tokens_arg={num_cot_tokens}, will loop cot_tokens from 1 to {phase}")
    for cot_tokens in range(1, phase + 1):
        targets, modified_inputs = compute_parity(eval_inputs, cot_tokens, subset_indices_arr=subset_indices_arr, coefficients=coefficients)
        outputs, loss = model.generate(modified_inputs, num_cot_tokens=cot_tokens, target=targets)
        losses_by_cot[cot_tokens] = loss.item()
        if cot_tokens == phase:
            #print(f"  [EVAL DEBUG] At cot_tokens={cot_tokens} (== phase): k_for_parity={cot_tokens}, num_cot_for_generate={cot_tokens}, loss={loss.item():.4f}")
            outputs_phase = outputs
            targets_phase = targets
            modified_inputs_phase = modified_inputs
    
    loss = losses_by_cot[phase]
    #print(f"  [EVAL DEBUG] Final reported loss = losses_by_cot[{phase}] = {loss:.4f}")
